...
const ConfirmSendModal = ({ show, onClose, onConfirm, isSubmitting, values, resolvedName }) => {
    const { amount, accountNumber, description } = values

    return (
        <ModalShell show={show} onClose={onClose} title="⚠️ Confirm Transfer">
            ...
            <div style={{
                ...
                <h4 style={{ color: primary, fontWeight: "bold", fontSize: "24px", margin: "0 0 6px" }}>
                    ₦{parseFloat(amount).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                </h4>
                <p style={{ color: "#888", fontSize: "13px", margin: 0 }}>
                    to <strong style={{ color: primary }}>{resolvedName || accountNumber}</strong>
                </p>
                {description && (
                    <p style={{ color: "#aaa", fontSize: "12px", marginTop: "4px", fontStyle: "italic" }}>
                        "{description}"
                    </p>
                )}
            </div>
